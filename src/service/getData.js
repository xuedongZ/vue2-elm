...
export const foodCategory = (latitude, longitude) =>
  fetch('GET', '/shopping/v2/restaurant/category', {
    latitude,
    longitude
  })
...
export const foodDelivery = (latitude, longitude) =>
  fetch('GET', '/shopping/v1/restaurants/delivery_modes', {
    latitude,
    longitude,
    kw: ''
  })
...
export const foodActivity = (latitude, longitude) =>
  fetch('GET', '/shopping/v1/restaurants/activity_attributes', {
    latitude,
    longitude,
    kw: ''
  })
...
export const getRatingList = (offset, tag_name = '') =>
  fetch('GET', '/ugc/v2/restaurants/834828/ratings', {
    has_content: true,
    offset,
    limit: 10,
    tag_name
...
export const sendLogin = (code, mobile, validate_token) =>
  fetch('POST', '/v1/login/app_mobile', {
    code,
    mobile,
    validate_token
  })
...
export const getAddress = (id, sig) =>
  fetch('GET', '/v1/carts/' + id + '/addresses', {
    sig
  })
...
export const postAddAddress = (userId, address, address_detail, geohash, name, phone, phone_bk, poi_type, sex, tag, tag_type) =>
  fetch('POST', '/v1/users/' + userId + '/addresses', {
    address,
    address_detail,
    geohash,
    name,
...
export const validateOrders = ({ user_id, cart_id, address_id, description, entities, geohash, sig, validation_code, validation_token }) =>
  fetch('POST', '/v1/users/' + user_id + '/carts/' + cart_id + '/orders', {
    address_id,
    come_from: 'mobile_web',
    deliver_time: '',
...
export const getOrderList = (user_id, offset) =>
  fetch('GET', '/bos/v2/users/' + user_id + '/orders', {
    limit: 10,
    offset
  })
